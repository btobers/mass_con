...
for ela in {1000..1800..10}
do
   counter=$[$counter + 1]
   python FlowlineMassCon.py config.ini $plot -mb $mb -ela $ela -dhdt $dhdt -gamma $gamma -out_name "${out_path}ela/gorge_animation/orig_frames/mb_${mb}_ela_${ela}_dhdt_${dhdt}_gamma_$gamma.csv" -direction updown
